...
theta0 = [
    ...
    np.log10(3e-4),  np.log10(45/1440), -8.0, 1.5,
]
...
def plot_fit(ax, sampler, mask_used, label, chi2_r, n_draws=200):
    ...
    p2_best = sbpl(t_model, params_best[4], params_best[5],
    ...
    ax.plot(t_model, np.where(p2_best>0, p2_best, np.nan), ':',
            ...
            label=f'P2 tb={params_best[5]*1440:.1f} min'
                  f'  α=({params_best[6]:.2f},{params_best[7]:.2f})')
...
try:
    ...
    samples_phys_B[:, 4] = 10**samples_phys_B[:, 4] * 1e6
    samples_phys_B[:, 5] = 10**samples_phys_B[:, 5] * 1440
    with plt.rc_context({'text.usetex': True}):
        fig_c = corner.corner(
            samples_phys_B,
            labels=[r'$F_{0,1}$ ($\mu$Jy)', r'$t_{\rm break,1}$ (min)',
                    r'$\alpha_{1,1}$', r'$\alpha_{2,1}$',
                    r'$F_{0,2}$ ($\mu$Jy)', r'$t_{\rm break,2}$ (min)',
